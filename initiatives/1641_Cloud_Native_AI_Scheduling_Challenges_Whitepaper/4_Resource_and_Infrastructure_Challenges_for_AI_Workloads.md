...
* **Solution approaches:**  
  ...
  * [**Dynamic Resource Allocation (DRA)**](https://kubernetes.io/docs/concepts/scheduling-eviction/dynamic-resource-allocation/) is a Kubernetes feature that allows flexible resource allocation, including GPU partitioning on demand which need to be implemented via vendor DRA drivers. Dynamic Resource Allocation reached GA in Kubernetes 1.34.
* **Caveats:**  
...
* Additionally, alongside financial costs, power and energy budgets should also be considered as critical infrastructure constrainsts in energy-intensive AI workloads. Telemetry data, such as GPU utilization, power consumption, and job runtime metrics, can be leveraged to enable smarter scheduling decisions, optimize resource allocation, and enforce budget constraints more effectively. These challenges remains an area for future development.
...
Not all GPUs in all server are necessarily connected via high-speed links. In a  GPU dense server only certain pairs have NVLink connections. The scheduler must understand this topology.
